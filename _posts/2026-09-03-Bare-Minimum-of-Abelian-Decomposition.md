...
$$
\hat{A}=\mathcal{A}+\mathcal{C}, \quad \mathcal{A}\propto \mathfrak{n}\text{ and }\mathcal{C}=\frac{i}{g}[\mathfrak{n},\partial \mathfrak{n}].
$$
...
Knowing the asymptotic form of the gauge field, we can work out the asymptotic form of $F_ {\mu\nu}$, however we are more interested in the U(1) part, that is the field strength in the same direction of $\phi$,
...
We can find corresponding $A_ \mu$ at spatial infinity by solving
...
$$
\mathfrak{n}:= (\vec{e}^{i}\cdot\hat{n}) T^{i}.
$$

This is in effect replacing the basis $\vec{e}^{1,2,3}$ with $T^{1,2,3}$. Now, we can treat $\mathfrak{n}$ as an adjoint field, and act on it with the covariant derivative, just as we can act covariant derivative on $\mathfrak{g}$-valued scalar fields. Furthermore, as in the monopole case, we decompose the gauge field into two parts, one for which $D_ {\mu}\mathfrak{n}=0$, and the part that does not. 

With the help of the following matrix identities:
...
the solution comprises of the abelian part that is proportional to $\mathfrak{n}$, and the non-abelian part that is perpendicular to $\mathfrak{n}$. Make use of identity 

$$
[[\partial_ {\mu}\mathfrak{n},\mathfrak{n}],\mathfrak{n}] = \partial_ {\mu}\mathfrak{n},
$$

the non-trivial part of the solution for $A_ {\mu}$, call it $\mathcal{C}_ {\mu}$, reads
...
Given a solution for $A$, we can add to it any thing proportional to $\mathfrak{n}$ and it is still a solution, since the commutator remains unchanged, hence the solutions contains a trivial part that is proportional to $\mathfrak{n}$, call it $\mathcal{A}$: 
...
Put together, the total solution $\hat{A}$ is called the `restricted gauge field`, 
...
If the symmetry is broken to the one-parameter subgroup $N$ generated by $\mathfrak{n}$, then $\mathcal{A}$ corresponds to the massless $U(1)$ component that can propagate for long distance. Geometrically, $\hat{A}$ is the component of the connection that leaves $\mathfrak{n}$ invariant during parallel transport. 
...
Since $\partial \hat{n}$ is perpendicular to $\hat{n}$ ($\hat{n}$ has fixed length), $\partial \hat{n}=(\cdots)\hat{n}_ {1}+(\cdots)\hat{n}_ {2}$. In $\mathfrak{g}$-space we have the same. Write 
...
- $A$ is the total $SU(N)$ field. This is sometimes denoted $\vec{A}$ when the gauge group is $SU(2)$, since $A=\vec{A}\cdot \vec{T}$, where $\vec{T}=(T^{1,2,3})$ are the generators of SU(2).
- $\mathcal{A}$ is the free, massless $U(1)$ gauge field, so-called *neuron*.
- $\mathcal{C}$ is the monopole-like background field defined(fixed) by $\hat{n}$, where $\hat{n}$ for example can be chosen as the hedgehog-form, similar to the monopole field configuration. Possible singularity points are allowed. It do not fluctuated and has no dynamics. It is merely a choice of frame.
...
Let's focus on $\mathcal{A}$ first. It is the $\mathfrak{n}$-direction projection of $\hat{A}$
...
Note that it is no longer a total derivative, but the projection of a total derivative in $\hat{n}$ direction. It is not simply $\mathcal{A}\to \Omega \mathcal{A}\Omega ^{\dagger}$, $\mathcal{C}\to \Omega(\mathcal{C}+i\partial)\Omega ^{\dagger}$.

Another way to write the infinitesimal form of $\delta a$ in terms of $\mathcal{C}$ is as follows. Write the gauge transformation parameter as 

$$
\alpha = \alpha^{(1)} \mathfrak{n}_ {1} + \alpha^{(2)} \mathfrak{n} _ {2} + \theta \mathfrak{n} .
$$

Since $\mathcal{C}=-\frac{1}{g}\hat{n}\times \partial \hat{n}$, it is orthodox to $\hat{n}$, write it in components of $\hat{n}_ {1}$ and $\hat{n}_ {2}$: (The minus is for bookkeeping reasons)

$$
\mathcal{C}=: -\mathcal{C}^{(1)}\hat{n}_ {1} - \mathcal{C}^{(2)} \hat{n}_ {2},
$$

then

$$
\mathcal{C}^{(2)} = -\hat{n}_ {2}\cdot \mathcal{C} 
= \frac{1}{g}\hat{n}_ {2}\cdot(\hat{n}\times \partial \hat{n}) 
= \frac{1}{g} \hat{n}\cdot(\partial \hat{n} \times \hat{n}_ {2})
=\frac{1}{g}\hat{n}\cdot(\partial(\hat{n}\times \hat{n}_ {2})-\hat{n}\times \partial \hat{n}_ {2}) 
= -\frac{1}{g}\hat{n}\cdot \partial \hat{n}_ {2},
$$

and similarly 

$$
\mathcal{C}^{(1)} = \frac{1}{g} \hat{n}\cdot \partial \hat{n}_ {2}.
$$

hence we have 

$$
\delta a = \frac{1}{g} \partial \theta - \alpha^{(1)}  \mathcal{C}^{(2)} + \alpha^{(2)} \mathcal{C}^{(1)}.
$$

define $C^{\pm}:= \frac{1}{\sqrt{2}}(\mathcal{C}^{(1)}\pm i\mathcal{C}^{(2)})$, we have 

$$
\delta a = \frac{1}{g} \partial \theta + i C^{+} \frac{1}{\sqrt{2}}(\alpha^{(1)}-i\alpha^{(2)}) - \frac{i}{\sqrt{2}} C^{-}(\alpha^{(1)} + i \alpha^{(2)}).
$$

define 

$$
\alpha^{\pm } := \frac{1}{\sqrt{2}}(\alpha^{(1)}\pm i \alpha^{(2)}),
$$

we have 

$$
\delta a = \frac{1}{g} \partial \theta + iC^{+}\alpha^{-} - i C^{-}\alpha^{+}.
$$
...
$$
\frac{2i}{g} \mathfrak{n} \partial \mathfrak{n}  \to \frac{2i}{g} \mathfrak{n}' \partial \mathfrak{n}' = \frac{2i}{g} \Omega \mathfrak{n} \Omega ^{\dagger}\partial(\Omega \mathfrak{n}  \Omega ^{\dagger})
$$

which is 

$$
\Omega \frac{2i}{g} \mathfrak{n} \partial \mathfrak{n} \Omega ^{\dagger} + \frac{i}{2g} \Omega \partial \Omega ^{\dagger} - \frac{2i}{g} (\Omega \mathfrak{n} \partial \Omega ^{\dagger})(\Omega \mathfrak{n} \Omega ^{\dagger}).
$$

The question is, is this equivalent to another way to determine the gauge transformation of $\mathcal{C}$:
...
showing that indeed the gauge transformed $\mathcal{C}'=\mathcal{C}$. The finite case is more complicated, we have 
...
$$
\begin{align*}
U \partial n U^{\dagger} &= i 2 \mathfrak{n} \partial \mathfrak{n}  \sinh k + \partial \mathfrak{n}  \cosh k ,\\
\partial U U^{\dagger} &= \int_ {0}^{1} ds U^{1-s}\partial(ikn) (U^{\dagger})^{1-s}.  
\end{align*}
$$

- - -

Regarding $X$ in $A = \mathcal{A}+\mathcal{C}+X$, it also transforms as a adjoint scalar,

$$
X\to \Omega X \Omega ^{\dagger}.
$$

How the field transforms tells us what gauge invariant terms can be constructed in the Lagrangian. For instance, we know that $\left\lvert \hat{D}_ {\mu}X \right\rvert^{2}$ is gauge invariant, so they could appear in the Lagrangian. 
...
which in components that are proportional and perpendicular to $\mathfrak{n}$ reads

$$
\begin{align*}
\partial a_ {\mu} &= 0, \\
\hat{n}\times \partial^{2}\hat{n} - g a_ {\mu} \partial_ {\mu}\hat{n} &= 0.
\end{align*}
$$

This way of defining the gauge transformation is referred to as the `active gauge transformation`, since it actively changes the orientation of $\hat{n}$.

- - -

Another possible gauge symmetry, so-called `passive gauge transformation`, reads
...
Now let's include the final piece, the non-abelian fluctuation $X$. The total field strength $F_ {\mu \nu}$ is 
...
where $\hat{D}_ {\mu}\vec{X}_ {\nu} =(\hat{D}_ {\mu}\vec{X}_ {\nu})^{a}T^{a}$, $T^{a}$ is the basis of $\mathfrak{su}(2)$. The right-hand side of the Euler-Lagrange equation reads
...
$$
\partial_ {\mu}X_ {\mu \nu} = g[(\partial_ {\mu}\hat{n})\cdot(\vec{X}_ {\mu}\times \vec{X}_ {\nu})+\hat{n}\cdot \partial_ {\mu}(\vec{X}_ {\mu}\times \vec{X}_ {\nu})],
$$

but $(\partial_ {\mu}\hat{n})\cdot(\vec{X}_ {\mu}\times \vec{X}_ {\nu})$ is the volume expanded by these three vectors, and these three vectors all lie in the same plane since they are all perpendicular to $\hat{n}$, so the volume is zero! Hence we have 

$$
\partial_ {\mu}X_ {\mu \nu} = g \hat{n} \cdot \partial_ {\mu}(\vec{X}_ {\mu}\times \vec{X}_ {\nu}).
$$

The same argument goes for $H_ {\mu \nu}=-\frac{1}{g} \hat{n}\cdot(\partial_ {\mu}\hat{n}\times \partial_ {\nu}\hat{n})$.

We can put all the terms proportional to the coupling $g$ to the right of the equation, 
...
$$
\partial_ {\mu}(H_ {\mu \nu}) = -\frac{1}{g}\hat{n}\cdot \partial_ {\mu}(\partial_ {\mu}\hat{n}\times \partial_ {\nu}\hat{n}) = -g\hat{n}\cdot[\vec{X}_ {\mu}\times (\hat{D}_ {\mu}\vec{X}_ {\nu}-\mu \leftrightarrow \nu)+\partial_ {\mu}(\vec{X}_ {\mu}\times \vec{X}_ {\nu})].
$$

hence

$$
\hat{n}\cdot \partial_ {\mu}(\partial_ {\mu}\hat{n}\times \partial_ {\nu}\hat{n}) = g^{2}\hat{n}\cdot[\vec{X}_ {\mu}\times (\hat{D}_ {\mu}\vec{X}_ {\nu}-\mu \leftrightarrow \nu)+\partial_ {\mu}(\vec{X}_ {\mu}\times \vec{X}_ {\nu})].
$$
...
We will use two different notations for indices associated with different basis, we will adopt Greek indices in parenthesis with basis $\hat{n}$：

$$
\vec{X} = X^{i} T^{i} = X^{(\alpha)}\hat{n}_ {\alpha},
$$

where the Greek letters take value in $\left\lbrace 1,2 \right\rbrace$, since in terms of $\hat{n}_ {i}$, $X$ only has $\hat{n}_ {1}$ and $\hat{n}_ {2}$ components. Write 
...
Regarding the covariance derivative. In general, covariant derivative encodes information about the curvature of the underlying space. If we are dealing with the $\hat{n}$-basis which is curvilinear, then the covariant derivative should be able to reflect this fact. We find that
...
Next let's consider a special case just to verify our result above. *If $\hat{n}$ is independent of spacetime,* then without loss of generality we can assume that $\hat{n}$ lies in the $T^{3}$ direction, $\hat{n}_ {1}=T^{1}$ and $\hat{n}_ {2} = T^{2}$ respectively in $\mathfrak{su}(2)$. $X=X^{(1)}\mathfrak{n}_ {1}+X^{(2)}\mathfrak{n}_ {2}$ becomes $X=X^{1}T^{1}+X^{2}T^{2}$. $\hat{A}=a_ {\mu}T^{3}$ since $\mathcal{C}$ vanishes, and as a consequence so does $H_ {\mu \nu}$.
...
Let $f:M\to N$ be a continuous map from mainifold $M$ to manifold $N$. To simplify the discussion we assume that both the manifold have the same volume, for example they could both be unit circles. To calculate the winding number of $f$, given a volume spaned by $\epsilon_ {1}V_ {1}, \epsilon_ {2}V_ {2},\cdots$ on $M$, we want to know how $f$ maps it to another volume in $N$, then integrate the total volume. If the total volume of $M$ is $A$, the projected volume is $nA$ where $n$ is an integer, then the winding number is $n$. In other words, we need to integrate the pushforward of $M$ to $N$. The volume is measured by the volume form, let $dV_ {N}$ be the volume form on $N$, the mapped volume would be $\int_ {N}(f_ {\ast}M) dV_ {N}$. However it is usually easier to calculate it on $M$, hence we instead pullback $dV_ {N}$ to $M$, and calculate $\int_M f^{\ast}dV_ {N}$. This will give the expression of winding number as an integral on physical space. In our case, $M$ and $N$ are both $\mathbb{S}^{2}$, the map is the field itself $\phi$, and the winding number is given by 
