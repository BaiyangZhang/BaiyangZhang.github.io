...
The background $\mathcal{C}$ together with $\mathcal{A}$ is the so-called restricted gauge field that is bonded to the "Higgs" field $\mathfrak{n}$, it is better to make it gauge transform accordingly, so under gauge transform

$$
\hat{A}\to \hat{A}'=\Omega(\hat{A}+i\partial)\Omega ^{\dagger}.
$$

What about the gauge transform of $\mathcal{A}$ and $\mathcal{C}$ respectively? 

If we want $\mathcal{C}$ to transform together with $\mathfrak{n}$, that is, let $\mathcal{C}$ be the nontrivial solution to $\mathcal{D}$

$$
\mathcal{A}\to \mathcal{A}'= \Omega \mathcal{A}\Omega ^{\dagger},
$$
